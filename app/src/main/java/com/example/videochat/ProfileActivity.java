...
public class ProfileActivity extends AppCompatActivity {
    ...
    private Button add_friend, decline_request;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Creating friend request send/decline..
        manageClickEvents();

    }
    private void manageClickEvents(){

        //if user click back while sending the req then retrieving the request type

        friendRequestRef.child(senderUserId).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                //if the sender has the receiver id then this is being executed...
                if(dataSnapshot.hasChild(receiverUserID)){
                    ...
                    if(requestType.equals("sent")){
                        ...
                        //display decline request button
                        decline_request.setVisibility(View.VISIBLE);

                        //if user clicked decline button
                ...
                //if the sender doesn't contain receiver id(means request is not received)
                else{
                    contactsRef.child(senderUserId).addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(DataSnapshot dataSnapshot) {
                            //if the sender has the receiver id
                            if(dataSnapshot.hasChild(receiverUserID)){
                                currentState = "friends";
                                add_friend.setText("Delete Contact");
                            }
                            //if the sender doesn't contains the receiver id(i.e Both are new to each other )
                            else{
                                currentState = "new";
                            }
                        }
        ...
        //if sender and receiver both are same
            ...
            add_friend.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if(currentState.equals("new")){
                        //send friend request to another person
                        SendFriendRequest();
                    }
                    //if  a person already sent friend request
                    if(currentState.equals("request_sent")){
                        //Canceling Friend Request while req is sent.
                        CancelFriendRequest();
                    }
                    //the receiver will see accept friend request btn when request is sent to him.
                    if(currentState.equals("request_received")){
                        AcceptFriendRequest();
                    }
                    //after accepting request and being friends
                    if(currentState.equals("request_sent")){
                        CancelFriendRequest();
    ...
    private void SendFriendRequest() {
        ...
        friendRequestRef.child(senderUserId).child(receiverUserID).child("request_type").setValue("sent")
                .addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        //receiver received req from sender.
                        if(task.isSuccessful()){
                        friendRequestRef.child(receiverUserID).child(senderUserId).child("request_type").setValue("received")
                                .addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if(task.isSuccessful()){
                                            currentState = "request_sent";
                                            //changing add friend btn to cancel req while sender sent friend req successfully.
                                            add_friend.setText("Cancel Friend Request");
